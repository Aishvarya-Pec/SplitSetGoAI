...
import { brandTemplate } from "@/lib/email/templates";
        ...
        users.map(async (u) => {
          ...
          const html = brandTemplate({
            ...
            bodyHtml: `
              ...
              <table cellspacing="0" cellpadding="0" border="1" style="border-collapse:collapse;">
                <thead>
                  <tr><th align="left">To</th><th align="right">Amount</th></tr>
                </thead>
                <tbody>${rows}</tbody>
              </table>
              <p><a class="btn" href="${process.env.NEXT_PUBLIC_BASE_URL || "https://splitsetgo.app"}">Open SplitSetGO</a></p>
            `,
          });
          ...
          try {
            await convex.action(api.email.sendEmail, {
              to: u.email,
              subject: "You have pending payments on SplitSetGO",
              html,
